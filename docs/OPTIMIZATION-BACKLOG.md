...
- **发布流水线：原生安装包（2026-09-12，`.github/workflows/release.yml`）**：由原来只产 app-image 绿色包，扩展为「绿色包 + 原生安装包」。Windows 产 `.msi`、Linux 产 `.deb`/`.rpm`、macOS 产 `.dmg`，各平台另附 `app-image` 绿色包与跨平台 fat jar。要点：打包前把 fat jar 单独 stage（避免把整个 `target/` 塞进镜像）；`--add-modules` 显式给出运行时模块集（GUI/JDBC/网络/内置 HttpServer/脚本/管理/加密/字符集/本地化，**必须单行**，YAML 折行会混入空格）；`--app-version` 用从 tag 提取的纯数字 `x.y.z`；安装包步骤全部 `continue-on-error`，某平台缺 WiX/`rpm` 时自动降级为只发绿色包而不阻断发布。发布阶段只打包不跑测试（测试由 `ci.yml` 把关）。
